...
export async function runDatabaseExport(mode: "catalog" | "lake" = "catalog", customOut?: string): Promise<string> {
  ...
  // Create Schema V2 tables in catalog
  catDb.run(`
  ...
  catDb.run(`
    CREATE TABLE package_fronts_v2 (
      id TEXT PRIMARY KEY,
  ...
  // Stream canonical_packages_v2 in chunks
  const srcDb = dbV2.rawDb;
  const packages = srcDb.query("SELECT * FROM canonical_packages_v2;").all() as any[];
  logger.info(`[Exporter] Copying ${packages.length} canonical packages...`);

  const insertPkg = catDb.prepare(`
    INSERT INTO canonical_packages_v2 (
  ...
  catDb.transaction(() => {
    for (const p of packages) {
      insertPkg.run(
        ...
        p.origin_created_at, p.origin_updated_at, p.created_at, p.updated_at
      );
    }
  })();

  // Copy fronts
  const fronts = srcDb.query("SELECT * FROM package_fronts_v2;").all() as any[];
  logger.info(`[Exporter] Copying ${fronts.length} package storefronts...`);

  const insertFront = catDb.prepare(`
    INSERT INTO package_fronts_v2 (
