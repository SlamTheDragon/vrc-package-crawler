    ...
    try {
      ...
      if (resp.status === 404) {
        ...
        if (creatorName && productSlug) {
            ...
            if (creatorResp.ok) {
              ...
              if (similar) {
                ...
                if (fullAlt !== currentUrl) {
                  ...
                  return crawlProduct(runtime, fullAlt);
                }
              }
            }
          } catch (_) {}
          ...
          const searchUrls = await crawlBrowsePage(runtime, `https://jinxxy.com/market/browse?query=${encodeURIComponent(productSlug.replace(/[-_]/g, " "))}`);
