...
async function runLiveMonitor() {
  while (isRunning) {
    ...
    for (const [p, stats] of Object.entries(metrics.platformStats)) {
      const s = stats as PlatformMetrics;
    ...
    console.log("\x1b[36m---------------------------------------------------------------------------------\x1b[0m");
    console.log("\x1b[1m Pre-Indexed Semantic Categories (canonical_packages):\x1b[0m");
